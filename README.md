...
**To create the separate repo** so Argo CD can use `sourceRepo: git@github.com:jd4883/homelab-prometheus.git` and `path: "."`, run from the **home repo root** (with `GH_TOKEN` from 1Password):

```bash
GH_HOST=github.com op run --env-file=homelab/.env.gh -- ./homelab/helm/create-prometheus-chart-repo.sh
```

This creates **jd4883/homelab-prometheus** on GitHub (if missing), pushes this chart on a feature branch, and opens a PR. After the PR is merged, Argo CD will sync from that repo.
